...
browser.webRequest.onBeforeRedirect.addListener(requestRedirected, {
    urls: ['<all_urls>']
});
browser.webRequest.onCompleted.addListener(requestDone, {
    urls: ['<all_urls>']
});
...
function requestStart(req) {
    if (req.tabId < 0) {
        console.log("No Valid Request")
        return;
    }
    req._startTime = Date.now();
    _requests[req.requestId] = req;
    _recentRequests.push(req);
    console.log("requestStart:", req, _requests, _recentRequests)
}

function requestHeadersReceived(req) {
    reqInfo = _requests[req.requestId];
    if (!reqInfo) {
        return;
    }
    reqInfo.noTimeout = true;
    console.log("requestHeadersReceived:", req, _requests, _recentRequests)
}

function requestRedirected(req) {
    var url;
    url = req.redirectUrl;
    if (!url) {
        return;
    ...
    console.log("requestRedirected:", req, _requests, _recentRequests)
}

function requestDone(req) {
    // delete _requests[req.requestId];
    console.log("requestDone:", req, _requests, _recentRequests)
}
...
function requestError(req) {
    ...
    console.log("requestError1:", req, _requests, _recentRequests)
    if (req.tabId < 0) {
        return;
    }
    ...
    console.log("requestError2:", req, _requests, _recentRequests)
}
