...
function collapseFailedResourcesIfNeeded() {
    ...
    var firstHeight = document.querySelector(".infoContainer").offsetHeight;

    let modules = document.querySelectorAll(".infoContainer");
    let length = modules.length;
    if (length > 1) {
        document.querySelector(".contentArea").onmousedown = function () {
            if (failedResourcesCollapsed) {
    ...
    failedResourcesCollapsed = length > 2;
    modules.forEach(function (module, index, parent) {
        module.style.height = `${firstHeight}px`;
        module.style.position = "absolute";
        module.style.zIndex = modules.length - index;
        module.classList.add("noselect");
        module.querySelector(".footer").hidden = index == 0;

        let scale = 1 - index / 20;
        let marginTop = 10 * index;
        console.log("marginTop:", index, marginTop);
